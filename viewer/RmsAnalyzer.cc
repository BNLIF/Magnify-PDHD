...
void RmsAnalyzer::AnalyzeFile(const char* inFile)
{
    ...
    // Discover APA suffix from Trun tree (branch name in file is "anodeNo")
    int apaNo = 0;
    TTree* trun = (TTree*)f->Get("Trun");
    if (trun) {
        if (trun->GetBranch("anodeNo")) trun->SetBranchAddress("anodeNo", &apaNo);
        if (trun->GetEntries() > 0) trun->GetEntry(0);
    }
    TString suf = TString::Format("%d", apaNo);
